...
class BjdaSpider(scrapy.Spider):
    name = "bjda_spider_203"

    def start_requests(self):
        req_dict_list = [

            {
                "url": "http://xzsp.bjfda.gov.cn/bfdaww/gmprz/gmprzAction!gzfwQueryList.dhtml",
                "totalrows": "211", "qyfl": "2", "qyfldm": "203", "firstFlag": "clear",
                "callback": self.parse_203
            },
    ...
    def parse_203(self, response):
        onclicks = response.xpath("//td/img/@onclick").extract()
        for onclick in onclicks:
            id = onclick.split('\'')[1]
            url = "gmprzAction!gzfwView.dhtml?gmprzModel.zjid=" + id

            url = response.urljoin(url)
            req = scrapy.Request(url, callback=self.parse_company_203)
            yield req

    def parse_company_203(self, response):
        # print response.body
        companyInfoItem = CompanyInfoItem()

        companyInfoItem['item_category'] = u"GMP认证企业"
        companyInfoItem['item_category_num'] = 203

        companyInfoItem['company_name'] = response.xpath('//table[3]/tr[1]/td[2]/text()').extract_first()
        companyInfoItem['license_number'] = response.xpath('//table[3]/tr[9]/td[4]/text()').extract_first()

        # companyInfoItem['business_type'] = response.xpath('//table[2]/tr[2]/td[2]/text()').extract_first()
        companyInfoItem['business_address'] = response.xpath('//table[3]/tr[10]/td[2]/text()').extract_first()

        companyInfoItem['date_of_issue'] = response.xpath('//table[3]/tr[12]/td[2]/text()').extract_first()
        companyInfoItem['operating_period'] = response.xpath('//table[3]/tr[12]/td[4]/text()').extract_first()
        ...
        #
        # companyInfoItem['business_scope'] = response.xpath('//table[2]/tr[7]/td[2]/text()').extract_first()
