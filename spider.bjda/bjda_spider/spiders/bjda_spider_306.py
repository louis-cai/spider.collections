...
class BjdaSpider(scrapy.Spider):
    name = "bjda_spider_306"

    def start_requests(self):
        req_dict_list = [

            {
                "url": "http://xzsp.bjfda.gov.cn/bfdaww/ylqxcpzcxx/ylqxcpzcxxAction!gzfwQueryList.dhtml",
                "totalrows": "6707", "qyfl": "3", "qyfldm": "306", "firstFlag": "clear",
                "callback": self.parse_306
            },
    ...
    def parse_306(self, response):
        onclicks = response.xpath("//td/img/@onclick").extract()
        for onclick in onclicks:
            id = onclick.split('\'')[1]
            url = "ylqxcpzcxxAction!gzfwView.dhtml?ylqxcpzcxxModel.xxbzj=" + id

            url = response.urljoin(url)
            req = scrapy.Request(url, callback=self.parse_company_306)
            yield req

    def parse_company_306(self, response):
        # print response.body
        companyInfoItem = CompanyInfoItem()

        companyInfoItem['item_category'] = u"医疗器械产品注册信息"
        companyInfoItem['item_category_num'] = 306
        ...
        companyInfoItem['date_of_issue'] = response.xpath('//table[3]/tr[4]/td[2]/text()').extract_first()
