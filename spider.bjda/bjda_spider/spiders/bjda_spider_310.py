...
class BjdaSpider(scrapy.Spider):
    name = "bjda_spider_310"

    def start_requests(self):
        req_dict_list = [

            {
                "url": "http://xzsp.bjfda.gov.cn/bfdaww/ylqxscxkeslqy/ylqxscxkeslqyAction!gzfwQueryList.dhtml",
                "totalrows": "884", "qyfl": "3", "qyfldm": "310", "firstFlag": "clear",
                "callback": self.parse_310
            },
    ...
    def parse_310(self, response):
        onclicks = response.xpath("//td/img/@onclick").extract()
        for onclick in onclicks:
            id = onclick.split('\'')[1]
            url = "/bfdaww/ylqxscxkeslqy/ylqxscxkeslqyAction!gzfwView.dhtml?qbid=" + id + "&sqlid=ylqxscxkeslqy.findQyjbxx&viewname=qyjbxx&qyfl=3"

            url = response.urljoin(url)
            req = scrapy.Request(url, callback=self.parse_company_310)
            yield req

    def parse_company_310(self, response):
        # print response.body
        companyInfoItem = CompanyInfoItem()

        companyInfoItem['item_category'] = u"医疗器械生产企业(二三类)"
        companyInfoItem['item_category_num'] = 310
        ...
        companyInfoItem['operating_period'] = response.xpath('//table[3]/tr[6]/td[4]/text()').extract_first()
