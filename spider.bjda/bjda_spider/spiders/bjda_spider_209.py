...
class BjdaSpider(scrapy.Spider):
    name = "bjda_spider_209"

    def start_requests(self):
        req_dict_list = [

            {
                "url": "http://xzsp.bjfda.gov.cn/bfdaww/ypgspqy/ypgspqyAction!gzfwQueryList.dhtml",
                "totalrows": "5735", "qyfl": "2", "qyfldm": "209", "firstFlag": "clear",
                "callback": self.parse_209
            },
    ...
    def parse_209(self, response):
        onclicks = response.xpath("//td/img/@onclick").extract()
        for onclick in onclicks:
            zj = onclick.split('\'')[1]
            url = "/bfdaww/ypgspqy/ypgspqyAction!gzfwView.dhtml?zj=" + zj + "&sqlid=ypgspqyxx.findQyjbxx&viewname=qyjbxx"

            url = response.urljoin(url)
            req = scrapy.Request(url, callback=self.parse_company_209)
            yield req

    def parse_company_209(self, response):
        # print response.body
        companyInfoItem = CompanyInfoItem()

        companyInfoItem['item_category'] = u"药品GSP企业"
        companyInfoItem['item_category_num'] = 209
        ...
        companyInfoItem['business_scope'] = response.xpath('//table[3]/tr[5]/td[4]/text()').extract_first()

        companyInfoItem['legal_representative'] = response.xpath('//table[3]/tr[6]/td[2]/text()').extract_first()
