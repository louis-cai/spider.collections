...
class BjdaSpider(scrapy.Spider):
    name = "bjda_spider_502"

    def start_requests(self):
        req_dict_list = [

            {
                "url": "http://xzsp.bjfda.gov.cn/bfdaww/hzpsc/hzpscAction!gzfwQueryList.dhtml",
                "totalrows": "245", "qyfl": "5", "qyfldm": "502", "firstFlag": "clear",
                "callback": self.parse_502
            },
    ...
    def parse_502(self, response):
        ...
        for onclick in onclicks:
            id = onclick.split('\'')[1]
            id1 = onclick.split('\'')[3]
            url = "/bfdaww/hzpsc/hzpscAction!gzfwView.dhtml?xkzid=" + id1 + "&xkzh=" + id + "&sqlid=hzpsc.findQyjbxx&viewname=qyjbxx"

            url = response.urljoin(url)
            req = scrapy.Request(url, callback=self.parse_company_502)
            yield req

    def parse_company_502(self, response):
        # print response.body
        companyInfoItem = CompanyInfoItem()

        companyInfoItem['item_category'] = u"化妆品生产企业"
        companyInfoItem['item_category_num'] = 502
